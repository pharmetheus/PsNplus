...
is_deeply(monitor::get_model_NMrun_translation($includes::testfiledir.'/monitor/modelfit_running/'),
		  {'CLACE2.mod' => 'NM_run1',
		   'CLCRCL5.mod' => 'NM_run2',
		   'CLDIG2.mod' => 'NM_run3',
		   'CLDIU2.mod' => 'NM_run4',
...
is_deeply(monitor::get_candidate_models($includes::testfiledir.'/monitor/scm_dir1/m1'),
		  {
...
is(monitor::maxevals_exceeded($includes::testfiledir.'/monitor/maxeval_exceeded.lst'),1,'maxeval exceeded');
is(monitor::maxevals_exceeded($includes::testfiledir.'/pheno_with_cov.lst'),0,'maxeval not exceeded');

cmp_ok(monitor::get_final_ofv($includes::testfiledir.'/monitor/finished.ext'),'==',-639.01802895383548,'final ofv');

my ($feval,$ofv) = monitor::get_current_feval_ofv($includes::testfiledir.'/monitor/finished.ext');

cmp_ok($feval,'==',48,'monitor feval');
cmp_ok($ofv,'==',-639.01802895383548,'monitor ofv');

($feval,$ofv) = monitor::get_current_feval_ofv($includes::testfiledir.'/monitor/modelfit_running/NM_run3/psn.ext');

cmp_ok($feval,'==',0,'monitor feval');
cmp_ok($ofv,'==',-634.64433830581231,'monitor ofv');

($feval,$ofv) = monitor::get_current_feval_ofv($includes::testfiledir.'/monitor/modelfit_running/NM_run3/psn.hej');

is($feval,undef,'monitor feval');
is($ofv,undef,'monitor ofv');

($feval,$ofv) = monitor::get_current_feval_ofv($includes::testfiledir.'/empty.ext');

is($feval,undef,'monitor feval empty');
is($ofv,undef,'monitor ofv empty');

is(monitor::get_scriptname($includes::testfiledir.'/monitor/'),'scmplus','get toolname scmplus');
is(monitor::get_scriptname($includes::testfiledir.'/monitor/scm_dir1'),'scm','get toolname scm');


my %need_options=('logfile' => undef, 'search_direction' => undef);
is(monitor::get_options(directory =>$includes::testfiledir.'/notthere/',
						options => \%need_options),0,'no logfilename');

is(monitor::get_options(directory => $includes::testfiledir.'/monitor/',
...
is_deeply(monitor::get_scm_progress(tooldir=>$includes::testfiledir.'/monitor/active_scm_dir1'),\%active,'get scm progress');
